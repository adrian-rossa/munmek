...
- 🧠 **Multi-Stage Progressive Neural Reranker & Sense Preselector**:
  ...
  - **Stage 2 (Multilingual E5 INT8 ONNX, ~118 MB)**: Asynchronously computes cross-lingual bi-encoder vector embeddings mapping Korean sentence context directly to multilanguage definitions for semantic sense disambiguation. Includes a user toggle in options to disable/enable Stage 2 performance overhead, an in-memory session cache for instant re-hovers, and seamless `✨ Context Matched · XX%` badge updates with exact definition tab preselection.
...
- 🤖 **On-Demand Gemini AI & Quick LLM Lookups**: Click "Ask Gemini" inside the hover tooltip for context-aware grammar notes, clause analysis, and nuances. If no local dictionary entry matches, Quick LLM Lookup automatically provides concise contextual definitions and candidate explanations, featuring a 1-click `+ Anki` button for immediate flashcard creation.
